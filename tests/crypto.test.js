...
test('still decrypts payloads written by the pre-v1 release', async () => {
	...
	const legacy = await buildLegacyFixture();
 ...
 * (deriveBits -> importKey, 100,000 iterations, JSON + btoa). Generated at test
 * time rather than hard-coded so the fixture cannot drift from its own spec.
 */
async function buildLegacyFixture() {
	...
	const material = await crypto.subtle.importKey(
		'raw', encoder.encode('legacy-pass'), { name: 'PBKDF2' }, false, ['deriveBits']
	);
